<script lang="ts">
	import { deleteCustomer, getAllCustomer, getCustomerByID, updateCustomer } from '@/api/accounts/customers';
	...
	import { updateCustomerSchema, updateUserSchema, userSchema } from '@/constant/schema';
	...
	} = createEditModal<z.infer<typeof updateCustomerSchema>>({
		mutationApi: updateCustomer,
		refetch: $query ? $query.refetch : () => {},
		formSchema: updateCustomerSchema,
...
<AdminPageLayout pageName="Customers">
...
<!-- Edit Modal -->
<FormModal open={$isEditFormOpen} onCancel={closeEditModal} title="Edit Customer" form={editForm}>
