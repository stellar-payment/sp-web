<script lang="ts">
	import { getAdminDashboard, getCustomerDashboard, getMerchantDashboard } from '@/api/dashboard/api';
...
<DashboardLayout pageName="Dashboard">
	...
	<div class="flex flex-wrap gap-2 w-full mt-5">
		<DashboardCard
			title="Total Friend Transaction"
			quantity={$query.data?.peer_trx_count || 0}
			changes={0}
			icon={CreditCard}
		/>
		<DashboardCard
			title="Total Friend Transaction Value"
			quantity={$query.data?.peer_trx_nominal || 0}
			changes={0}
			isCurrency={true}
			icon={CreditCard}
		/>
		...
		/>
		<DashboardCard
			title="Total Merchant Transaction Value"
			quantity={$query.data?.merchant_trx_nominal || 0}
			changes={0}
			isCurrency={true}
			icon={CreditCard}
		/>
